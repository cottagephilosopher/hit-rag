...
**原因：** Ubuntu 系统的 Docker 容器驱动不支持 `host-gateway` 特性。

**解决方案：**

1. **使用 Ubuntu 专用配置（推荐）：**
   ```bash
   # 脚本会自动检测 Ubuntu 系统并使用 docker-compose.ubuntu.yml
   ./build.sh -p -s
   ```
...
2. **使用主机 IP 替代 host.docker.internal：**
   ...
   # 修改 docker-compose.ubuntu.yml 中的代理地址
...
2. **优化内存使用：**
   ```bash
   # 在 docker-compose.ubuntu.yml 中添加资源限制
   deploy:
     resources:
       limits:
         memory: 2G
...
```bash
# 查看构建日志
docker compose -f docker-compose.ubuntu.yml build --progress=plain

# 查看服务日志
docker compose -f docker-compose.ubuntu.yml logs -f backend

# 查看 Milvus 日志
docker compose -f docker-compose.ubuntu.yml logs -f milvus
```
...
3. 错误日志：`docker compose -f docker-compose.ubuntu.yml logs`
4. 系统信息：`uname -a`
